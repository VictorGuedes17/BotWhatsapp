  ...
  return await new Promise((resolve2, reject) => {
    db.run("CREATE TABLE IF NOT EXISTS trackConsumer (step TEXT, consumer TEXT)", (err) => {
      ...
      db.run("CREATE TABLE IF NOT EXISTS steps (orders INTEGER, identifier TEXT, description TEXT, parent TEXT, type TEXT, isLast INTEGER)", async (err) => {
        ...
        await new Promise((resolve, reject) => {
          db.run("INSERT INTO steps VALUES(2, '#sub002', 'Para rastrear o seu pedido é so acessar o link atraves do site dos correios link:www e inserir o codigo de rastreio á e lembrando, o envio leva de 3 a 5 dias uteis corridos, sabado e domingo não conta como dia uteis.', '#in001', 'msg', 0)", (err) => {
            if (err) reject(err);
            resolve(true);
          })
        });
